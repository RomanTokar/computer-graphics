...
const KochSnowflake = ({
  ...
  const draw = (
    ...
    if (points.length > 1) {
      if (zoom >= 0) {
        ...
        for (let i = 0; i < zoom; i++) {
          breacked = breakeLines(breacked, invertX, invertY);
        }

        drawByPoints(ctx, breacked, offset);
      }
    }
  };
  ...
  const breakeLines = (points = [], invertX = false, invertY = false) => {
    let result = [];

    for (let i = 0; i < points.length - 1; i++) {
      result = result.concat(
        brakeLine(points[i], points[i + 1], invertX, invertY)
      );
    }
